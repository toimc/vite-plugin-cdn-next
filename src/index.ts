...
function createDependency() {
  ...
  const filter = (
    ...
    lex: Awaited<ReturnType<typeof createRsModuleLexer>>
  ) => {
  ...
  return {
    ...
    get dependencyWithAlias() {
      const traverse = (aliases: string[], name: string) =>
        aliases.reduce((acc, cur) => ({ ...acc, [cur]: name }), {})
...
function cdn(opts: CDNPluginOptions = {}): Plugin[] {
        ...
        try {
          ...
          if (apply === 'serve') {
            ...
            config.optimizeDeps.include = include.filter(
              (dep) => !config.optimizeDeps.exclude.includes(dep)
            )
          }
        } catch (error) {
  ...
  return [
    { ...transformPlugin(), apply },
    { ...transformPresetModule(api), apply }
  ]
}
    ...
    async buildStart() {
      try {
        ...
        for (const module of modules) {
          if (!module.global) {
            throw new Error(`vite-plugin-external: missing global for module ${module.name}`)
          }
        }
        debug('check done')
        ...
        const dependencies = (await Promise.all(
          modules.map(async (module) => {
            ...
            return {
              bindings: exports,
              ...module,
              aliases: serializationExportsFields(module.name, module.aliases)
            }
          })
        )) as ModuleInfo[]
        dependency.dependency = dependencies.reduce(
          (deps, dep) => ({ ...deps, [dep.name]: dep }),
          {}
        )
