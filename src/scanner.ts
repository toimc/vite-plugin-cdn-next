...
function createWorkerThreads(scannerModule: ScannerModule, defaultWd: string) {
  ...
  const worker = new worker_threads.Worker(___filename, {
    workerData: {
      workerPort,
      internalThread: true,
      scannerModule: scannerModule.modules,
      defaultWd
    },
  ...
  const runSync = () => {
    ...
    if (message.id !== id) {
      throw new Error(`Internal error: Expected id ${id} but got id ${message.id}`)
    }
...
async function tryResolveModule(
  ...
  try {
    ...
    let { version, name, unpkg, jsdelivr } = packageJSON
    // 如果version, name为空，则从上级目录中的package.json中读取对应的信息
    if (!version || !name) {
      const parentPackageJsonPath = lookup(path.dirname(packageJsonPath), 'package.json')
      const parentStr = await fsp.readFile(parentPackageJsonPath, 'utf8')
      const parentPackageJSON = JSON.parse(parentStr)
      version = version || parentPackageJSON.version
      name = name || parentPackageJSON.name
      unpkg = unpkg || parentPackageJSON.unpkg
      jsdelivr = jsdelivr || parentPackageJSON.jsdelivr
    }
    const meta: ModuleInfo = Object.create(null)
    // 大多数包都有jsdelivr或unpkg字段
    // 但有一小部分没有，所以我们需要接受用户自定义
    ...
    if (rest.global) {
      ...
      Object.assign(meta, {
        name,
        version,
        relativeModule: iifeRelativePath,
        aliases: serializationExportsFields(name, aliases),
        ...rest
      })
    } else {
      // 如果不是全局模块，需要读取IIFE文件内容
