...
function isTopLevelCalled(p: NodePath) {
  return (
    ...
    t.isExportDefaultDeclaration(p.parent) ||
    t.isExportNamedDeclaration(p.parent)
  )
}
...
function externalGlobals(options: TransformWithBabelOptions): PluginTarget {
  return function () {
          ...
          case 'ImportSpecifier':
            if (specifier.imported.type === 'Identifier') {
              references.set(
                ...
                `${globalName}.${specifier.imported.name}.${aliase}`
              )
            }
        }
      }
    ...
    const overWriteExportNamedDeclaration = (path: NodePath<t.ExportNamedDeclaration>) => {
      const nodes: Array<
        t.VariableDeclarator | t.ObjectExpression | t.MemberExpression | t.Identifier
      > = []
      ...
      const scanNamedExportsWithSource = (
        l: t.Identifier,
        e: t.Identifier,
        ...
        if (l.name === 'default' && l.name !== e.name) {
          const memberExpression = (p) =>
            t.memberExpression(t.identifier(globalName), t.identifier(p))
          const objectExpression = [...bindings.keys()].map((p) =>
            t.objectProperty(t.identifier(p), memberExpression(p))
          )
      ...
      const scanNamedExportsWithoutSource = (
        l: t.Identifier,
        e: t.Identifier,
        specifier: t.ExportSpecifier
      ) => {
          ...
          const memberExpression = (() => {
            if (p === 'ImportNamespaceSpecifier') {
              ...
              return t.objectExpression(
                [...dependency[a].bindings.keys()].map((p) =>
                  t.objectProperty(t.identifier(p), memberExpression(p))
                )
              )
            }
      ...
      const variableDeclaratorNodes = nodes.filter(
        (node): node is t.VariableDeclarator => node.type === 'VariableDeclarator'
      )
      const objectOrMemberExpression = nodes.filter(
        (node): node is t.ObjectExpression | t.MemberExpression =>
    ...
    const eraseDuplicatedVariableDeclaration = (
      paths: NodePath<t.Node> | NodePath<t.Node>[],
      declarations: Map<string, NodePath<t.Declaration | t.Node>>
    ) => {
            ...
            switch (path.node.type) {
              case 'ExportDefaultDeclaration':
                ...
                if (
                  ...
                  references.has(path.node.declaration.name)
                ) {
                  ...
                  const variableDeclaration = t.variableDeclaration('var', [
                    t.variableDeclarator(t.identifier(id), t.identifier(globalName))
                  ])
  ...
  traverse(ast, {
    ...
    AssignmentExpression: (path) => {
      ...
      if (op.node.type === 'MemberExpression') {
        ...
        if (
          new RegExp(regName || 'global', 'i').test(code.slice(start, end)) &&
          t.isIdentifier(op.node.property)
        ) {
